-- Fix storage path handling issues causing 500 errors during file uploads

-- Replace the handle_storage_update function with a safer version that properly handles paths
...
begin
  ...
  BEGIN
    ...
    IF document_id IS NOT NULL THEN
      ...
      url_path := storage.foldername(new.id)::text || '/' || storage.filename(new.id)::text;
